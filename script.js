//decrypt(value, key); encrypt(value, key);
...
function encrypt(text, key) {
    let newkey = sha256(key);
    if (newkey.length <= text.length) {
        for (let i = 0; i < text.length; i = i + key.length) {
            newkey += key.repeat(1);
        }
    }
    key = newkey.substring(0, text.length);
    let tempkey = 0;
    let temptxt = 0;
    let result = "";
    for (let i = 0; i < text.length; i++) {
        tempkey = Math.round(key.charCodeAt(i) * 2);
        temptxt = text.charCodeAt(i);
        tempkey = tempkey + temptxt;
        if (tempkey <= 5) {
            tempkey += 30;
        }
        result += String.fromCodePoint(tempkey);
    }
    return result;
}
